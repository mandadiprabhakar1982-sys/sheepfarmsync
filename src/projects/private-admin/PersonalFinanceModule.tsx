...
export function PersonalFinanceModule() {
  const { 
    farmExpenses, monthlyIncomes, monthlyExpenses,
    addMonthlyIncome, addMonthlyExpense, isLoading, userRole 
  } = useFarm();
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8 bg-white rounded-[2rem] border-2 border-dashed border-slate-100">
        <ReceiptIndianRupee className="h-16 w-16 text-[#0FA5A0] mb-6 opacity-20" />
        <h2 className="text-2xl font-black uppercase tracking-tight text-slate-800">Personal Cash Flow</h2>
        <p className="text-slate-400 font-bold text-sm mt-2 max-w-sm">Detailed personal finance audits are restricted to the administrative identity. System stealth protocol active.</p>
      </div>
    );
  }
    ...
    <div className="animate-in fade-in duration-700 max-w-7xl mx-auto h-full flex flex-col relative px-4 md:px-0">
      <div className="space-y-6 flex-1 min-h-0 flex flex-col">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 flex flex-col min-h-0">
          <TabsList className="w-full h-14 md:h-16 bg-[#D7F2F1] rounded-2xl p-1.5 flex justify-start md:justify-center overflow-x-auto no-scrollbar shadow-inner mb-8">
            <TabsTrigger value="income"><ArrowUpCircle className="h-3.5 w-3.5 mr-2" /> Income</TabsTrigger>
            <TabsTrigger value="bank_emi"><Landmark className="h-3.5 w-3.5 mr-2" /> Bank EMI</TabsTrigger>
            <TabsTrigger value="card"><CreditCard className="h-3.5 w-3.5 mr-2" /> Card</TabsTrigger>
            <TabsTrigger value="personal"><User className="h-3.5 w-3.5 mr-2" /> Personal</TabsTrigger>
            <TabsTrigger value="household"><Home className="h-3.5 w-3.5 mr-2" /> Household</TabsTrigger>
          </TabsList>
          
          <TabsContent value={activeTab} className="flex-1 min-h-0 flex flex-col m-0">
            <div className="flex-1 min-h-0 flex flex-col premium-card overflow-hidden bg-white rounded-[2rem] shadow-xl">
                  ...
                  <Table>
                    <TableHeader className="bg-[#0FA5A0] sticky top-0 z-10">
                      <TableRow className="border-none hover:bg-transparent">
                        ...
                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-6 text-white">Description</TableHead>
                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-6 text-center text-white">Category</TableHead>
                    ...
                    <TableBody>
                      {combinedData.map((item) => (
                        <TableRow key={item.id} className="hover:bg-slate-50 border-b border-slate-100 transition-colors">
                          ...
                          <TableCell><div className="flex flex-col"><span className="text-[14px] font-black text-[#2F4F4F]">{item.source}</span></div></TableCell>
      ...
      <Dialog open={isIncomeDialogOpen} onOpenChange={setIsIncomeDialogOpen}>
        <DialogContent className="sm:max-w-xl rounded-[2rem] p-0 overflow-visible border-none shadow-2xl bg-white h-[88dvh] max-h-[88dvh] flex flex-col">
          <DialogHeader className="bg-neutral-900 p-8 text-left text-white shrink-0"><div className="flex items-center gap-3 mb-2"><div className="p-2.5 rounded-xl bg-[#0FA5A0]/20 text-[#0FA5A0]"><ArrowUpCircle className="h-5 w-5" /></div><DialogTitle className="text-xl font-black uppercase text-white">Income Entry</DialogTitle></div><DialogClose className="absolute right-6 top-6 text-white/40"><X className="h-5 w-5" /></DialogClose></DialogHeader>
          <div className="p-8 overflow-y-auto no-scrollbar flex-1">
            <div className="min-h-[500px] space-y-6">
              <div className="space-y-2"><Label className="text-[10px] font-black uppercase">Transaction Date</Label>
                <Popover open={isIncomeDatePickerOpen} onOpenChange={setIsIncomeDatePickerOpen}>
                  <PopoverTrigger asChild><Button variant="outline" className="h-14 rounded-2xl bg-slate-50 border-none font-bold w-full text-left justify-between">{entryDate ? format(entryDate, "MMM dd, yyyy") : "Pick date"}<CalendarIcon className="h-4 w-4 opacity-20" /></Button></PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start"><HorizontalDatePicker selectedDate={entryDate} onSelect={(date) => { setEntryDate(date); setIsIncomeDatePickerOpen(false); }} /></PopoverContent>
                </Popover>
              </div>
              <div className="space-y-2 mt-4"><Label className="text-[10px] font-black uppercase">Income Source</Label><Input placeholder="e.g. Salary, Rent" value={source} onChange={(e) => setSource(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
              <div className="space-y-2 mt-4"><Label className="text-[10px] font-black uppercase">Amount (₹)</Label><Input type="number" placeholder="0" value={amount} onChange={(e) => setAmount(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
            </div>
          </div>
      ...
      <Dialog open={isExpenseDialogOpen} onOpenChange={setIsExpenseDialogOpen}>
        <DialogContent className="sm:max-w-xl rounded-[2rem] p-0 overflow-visible border-none shadow-2xl bg-white h-[88dvh] max-h-[88dvh] flex flex-col">
          <DialogHeader className="bg-neutral-900 p-8 text-left text-white shrink-0"><div className="flex items-center gap-3 mb-2"><div className="p-2.5 rounded-xl bg-rose-500/20 text-rose-400"><ArrowDownCircle className="h-5 w-5" /></div><DialogTitle className="text-xl font-black uppercase text-white">Expense Entry</DialogTitle></div><DialogClose className="absolute right-6 top-6 text-white/40"><X className="h-5 w-5" /></DialogClose></DialogHeader>
          <div className="p-8 overflow-y-auto no-scrollbar flex-1">
            <div className="min-h-[500px] space-y-6">
              <div className="space-y-2"><Label className="text-[10px] font-black uppercase">Transaction Date</Label>
                <Popover open={isExpenseDatePickerOpen} onOpenChange={setIsExpenseDatePickerOpen}>
                  <PopoverTrigger asChild><Button variant="outline" className="h-14 rounded-2xl bg-slate-50 border-none font-bold w-full text-left justify-between">{entryDate ? format(entryDate, "MMM dd, yyyy") : "Pick date"}<CalendarIcon className="h-4 w-4 opacity-20" /></Button></PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start"><HorizontalDatePicker selectedDate={entryDate} onSelect={(date) => { setEntryDate(date); setIsExpenseDatePickerOpen(false); }} /></PopoverContent>
                </Popover>
              </div>
              <div className="space-y-2 mt-4"><Label className="text-[10px] font-black uppercase">Expense Detail</Label><Input placeholder="e.g. EB Bill, Groceries" value={source} onChange={(e) => setSource(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div className="space-y-2"><Label className="text-[10px] font-black uppercase">Amount (₹)</Label><Input type="number" placeholder="0" value={amount} onChange={(e) => setAmount(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
                <div className="space-y-2"><Label className="text-[10px] font-black uppercase">Ledger Category</Label><Select value={category} onValueChange={(v: any) => setCategory(v)}><SelectTrigger className="h-14 rounded-2xl bg-slate-50 border-none font-bold"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="loan">Bank EMI</SelectItem><SelectItem value="card">Credit Card</SelectItem><SelectItem value="private">Personal Spend</SelectItem><SelectItem value="household">Household</SelectItem></SelectContent></Select></div>
              </div>
            </div>
          </div>
