  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8 bg-white rounded-[2rem] border-2 border-dashed border-slate-100">
        ...
        <p className="text-slate-400 font-bold text-sm mt-2 max-w-sm">This portfolio contains sensitive personal liability data and is restricted to the master account holder.</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        <Card className="p-8 flex items-center gap-6"><div className="p-4 rounded-2xl bg-[#D7F2F1] text-[#0FA5A0] shadow-sm"><ReceiptIndianRupee className="h-7 w-7" /></div><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-1">Monthly EMI</p><p className="text-3xl font-black tracking-tighter text-[#2F4F4F]">₹{totalMonthlyEmi.toLocaleString()}</p></div></Card>
        <Card className="p-8 flex items-center gap-6"><div className="p-4 rounded-2xl bg-[#D7F2F1] text-[#0FA5A0] shadow-sm"><Landmark className="h-7 w-7" /></div><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-1">Bank Loans</p><p className="text-3xl font-black tracking-tighter text-[#2F4F4F]">₹{totalLoanBalance.toLocaleString()}</p></div></Card>
        <Card className="p-8 flex items-center gap-6"><div className="p-4 rounded-2xl bg-[#D7F2F1] text-[#0FA5A0] shadow-sm"><CreditCard className="h-7 w-7" /></div><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-1">Card Debt</p><p className="text-3xl font-black tracking-tighter text-[#2F4F4F]">₹{totalCreditCardDebt.toLocaleString()}</p></div></Card>
        <Card className="p-8 flex items-center gap-6"><div className="p-4 rounded-2xl bg-[#D7F2F1] text-[#0FA5A0] shadow-sm"><Banknote className="h-7 w-7" /></div><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-1">Private Debt</p><p className="text-3xl font-black tracking-tighter text-[#2F4F4F]">₹{totalPrivateDebt.toLocaleString()}</p></div></Card>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mb-10 p-1.5 bg-[#D7F2F1] rounded-2xl grid grid-cols-3 h-14 max-w-2xl mx-auto shadow-inner"><TabsTrigger value="loans">Bank Loans</TabsTrigger><TabsTrigger value="cards">Credit Cards</TabsTrigger><TabsTrigger value="private">Private Debt</TabsTrigger></TabsList>
        <TabsContent value="loans" className="m-0">
          <Card className="overflow-hidden bg-white"><CardHeader className="bg-[#0FA5A0] text-white p-10 py-12"><div className="flex justify-between items-end"><div className="space-y-1"><div className="flex items-center gap-3"><Landmark className="h-6 w-6" /><CardTitle className="text-2xl font-black tracking-tight leading-none uppercase text-white">Bank Loan Audit</CardTitle></div><CardDescription className="text-white/60 text-[8px] font-black uppercase tracking-[0.2em]">Real-time principal reduction tracking</CardDescription></div><p className="text-4xl font-black tracking-tighter">₹{totalLoanBalance.toLocaleString()}</p></div></CardHeader><div className="overflow-x-auto"><Table><TableHeader className="bg-[#0FA5A0] sticky top-0 z-10"><TableRow className="border-none hover:bg-transparent"><TableHead className="text-[10px] font-black uppercase py-6 pl-10 text-white">Bank Name</TableHead><TableHead className="text-[10px] font-black uppercase text-white">Repayment Progress</TableHead><TableHead className="text-[10px] font-black uppercase text-right text-white">Principal Balance</TableHead><TableHead className="text-[10px] font-black uppercase text-right text-white">Next EMI</TableHead></TableRow></TableHeader><TableBody>{sortedLoans.map((loan) => { const progress = loan.totalLoan > 0 ? ((loan.totalLoan - loan.balanceLoan) / loan.totalLoan) * 100 : 0; return (<TableRow key={loan.id} className="group hover:bg-slate-50 transition-colors border-b border-slate-100"><TableCell className="pl-10 py-8"><span className="text-[16px] font-black text-[#2F4F4F]">{loan.bankName}</span></TableCell><TableCell className="min-w-[180px]"><div className="space-y-2 py-2"><div className="flex justify-between text-[8px] font-black uppercase tracking-widest"><span className="text-[#2F4F4F]">{progress.toFixed(0)}% Repaid</span><span className="text-rose-600">{Math.round(loan.pendingTenure)} Mos Left</span></div><Progress value={progress} className="h-1.5 bg-neutral-100" indicatorClassName={getProgressClass(progress)} /></div></TableCell><TableCell className="text-right"><p className="text-[16px] font-black text-[#2F4F4F]">₹{loan.balanceLoan.toLocaleString()}</p><p className="text-[9px] font-bold text-muted-foreground uppercase opacity-40">Of ₹{loan.totalLoan.toLocaleString()}</p></TableCell><TableCell className="text-right text-[16px] font-black text-[#2F4F4F]">₹{loan.monthlyEmi.toLocaleString()}</TableCell></TableRow>); })}</TableBody></Table></div></Card>
        </TabsContent>
      </Tabs>
          ...
          <div className="p-8 overflow-y-auto no-scrollbar flex-1">
            <div className="min-h-[500px] space-y-6">
              {activeTab === 'loans' && (<div className="space-y-6">
                <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Bank Identity</Label><Input value={bankName} onChange={(e) => setBankName(e.target.value)} placeholder="Bank Name" className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
                <div className="grid grid-cols-2 gap-4 mt-4"><div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Total Loan (₹)</Label><Input type="number" value={totalLoan} onChange={(e) => setTotalLoan(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div><div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Interest Rate %</Label><Input type="number" value={interest} onChange={(e) => setInterest(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div></div>
                <div className="grid grid-cols-2 gap-4 mt-4">
                  ...
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Monthly EMI (₹)</Label><Input type="number" value={monthlyEmi} onChange={(e) => setMonthlyEmi(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
                </div>
              </div>)}
              {activeTab === 'cards' && (<div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  ...
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Total Limit (₹)</Label><Input type="number" value={cardTotalLimit} onChange={(e) => setCardTotalLimit(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
                </div>
                <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Outstanding (₹)</Label><Input type="number" value={cardOutstanding} onChange={(e) => setCardOutstanding(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold text-rose-600" /></div>
              </div>)}
              {activeTab === 'private' && (<div className="space-y-6">
                <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Person Name</Label><Input value={personName} onChange={(e) => setPersonName(e.target.value)} placeholder="Name" className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Amount (₹)</Label><Input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" /></div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest">Debt Date</Label>
