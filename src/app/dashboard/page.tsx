  ...
  const HubCard = ({ item }: { item: any }) => {
    return (
      <Link href={item.href} className="group transition-all active:scale-95">
        <div className="w-full h-[180px] sm:h-[240px] md:h-[280px] bg-white/80 backdrop-blur-xl rounded-[2.5rem] p-4 sm:p-8 flex flex-col items-center justify-center gap-3 sm:gap-6 shadow-xl hover:shadow-[0_20px_60px_rgba(6,78,59,0.15)] transition-all hover:-translate-y-2 border border-white relative overflow-hidden glass-sheen">
          ...
          <div className="relative w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 transition-transform group-hover:scale-110 duration-700 z-10 text-primary group-hover:text-white flex items-center justify-center">
            <item.icon className="w-full h-full" />
          </div>
          
          <div className="text-center relative z-10">
            <h3 className="text-[11px] sm:text-[13px] md:text-[15px] font-black text-slate-900 tracking-widest leading-none mb-1 uppercase group-hover:text-white transition-colors">{item.title}</h3>
  ...
  return (
    <div className="animate-in fade-in duration-1000 max-w-7xl mx-auto py-4 sm:py-8 px-4">
      <div className="flex items-center gap-6 sm:gap-10 mb-12 sm:mb-20">
        <HubSparkle className="h-16 w-16 sm:h-24 sm:w-24" />
        <div className="space-y-2 sm:space-y-3">
          <h1 className="text-2xl sm:text-5xl font-black text-slate-900 tracking-tighter uppercase leading-none">
            MPR <span className="text-primary">SHEEP FARMS</span>
          </h1>
          <div className="flex items-center gap-4">
            <span className="h-px w-12 bg-accent opacity-50 hidden sm:block" />
            <p className="text-[9px] sm:text-[11px] font-black text-primary/60 uppercase tracking-[0.5em] leading-none">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-10">
      ...
      <div className="mt-20 sm:mt-32 border-t border-slate-200 pt-10 opacity-40">
        <div className="flex justify-between items-center">
          <div>
            <p className="text-[10px] sm:text-[12px] font-black uppercase tracking-[0.5em] text-slate-900">MPR ENTERPRISE SYSTEM</p>
            <p className="text-[8px] sm:text-[9px] font-bold text-emerald-600 uppercase tracking-widest mt-2">V5.0.0 TACTICAL DEPLOYMENT</p>
          </div>
          <div className="flex items-center gap-4">
