...
export default function DashboardLayout({
  ...
  if (isLoadingProfile) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-black fixed inset-0 z-[9999]">
        <div className="flex flex-col items-center gap-6">
          <div className="w-12 h-12 border-4 border-white/10 rounded-full border-t-emerald-500 animate-spin" />
    ...
    <SidebarProvider>
      <div className="flex min-h-screen w-full bg-transparent overflow-hidden selection:bg-emerald-500/30 font-sans">
        ...
        <SidebarInset className="flex flex-col relative bg-transparent z-10">
          {/* Header Bar */}
          <header className="sticky top-4 z-50 flex h-16 items-center justify-between gap-4 bg-white/5 backdrop-blur-2xl mx-6 md:mx-10 mt-4 rounded-2xl border border-white/10 shadow-2xl px-6">
            <div className="flex items-center gap-4">
              <div className="md:hidden">
                <Logo showManager={false} light={true} className="scale-90 origin-left" />
              </div>
              <div className="hidden md:flex items-center gap-6">
                <SidebarTrigger className="h-10 w-10 hover:bg-white/5 rounded-xl transition-all text-white/60" />
                ...
                <div className="flex items-center gap-3 px-4 py-1.5 bg-white/5 rounded-lg border border-white/5">
                  ...
                  <h2 className="text-[10px] font-black text-white/40 tracking-[0.2em] uppercase">
            ...
            <div className="flex items-center gap-2 md:gap-4">
              <Button 
                ...
                className="rounded-xl hover:bg-white/5 text-white/60 transition-all"
              >
          ...
          <footer className="fixed bottom-0 left-0 right-0 z-50 md:hidden pb-[env(safe-area-inset-bottom)] px-4 mb-4">
            <nav className="flex h-16 items-center justify-around bg-black/40 backdrop-blur-3xl rounded-3xl shadow-2xl border border-white/10 px-2">
              {mobileNavItems.map((link) => {
                ...
                if (link.isCenter) {
                  return (
                    <Link key={link.href} href="/dashboard/livestock" className="relative -top-6 bg-emerald-600 text-white p-4 rounded-full shadow-2xl transition-transform active:scale-90 border-4 border-black/40">
