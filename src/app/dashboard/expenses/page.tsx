                  ...
                  <tbody>
                    {sortedFarmExpenses.map((e) => (
                      <tr key={e.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        <td className="py-6 px-4 text-[11px] font-black text-slate-400">{e.expenseDate}</td>
                        <td className="py-6 px-4"><span className="text-[14px] font-black text-slate-900">{e.description}</span></td>
                        <td className="py-6 px-4 text-right">
                          <div className="flex items-center justify-end gap-4">
                            <span className="text-[16px] font-black text-slate-900">₹{e.amount.toLocaleString()}</span>
                            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full text-rose-600 hover:bg-rose-50" onClick={() => deleteFarmExpense(e.id, e._path)}><Trash2 className="h-4 w-4" /></Button>
                          </div>
                        </td>
                      </tr>
                    ))}
