...
export default function ExpensesPage() {
  ...
  const { farmExpenses, addFarmExpense, deleteFarmExpense, totalFarmExpenses, isLoading } = useFarm();
  ...
  const sortedFarmExpenses = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => new Date(b.expenseDate).getTime() - new Date(a.date).getTime());
  }, [farmExpenses, searchTerm]);

  const groupedExpenses = useMemo(() => {
    const groups: { [key: string]: any[] } = {};
    sortedFarmExpenses.forEach(e => {
      if (!groups[e.expenseDate]) groups[e.expenseDate] = [];
      groups[e.expenseDate].push(e);
    });
    ...
    <div className="animate-in fade-in duration-700 max-w-7xl mx-auto h-full flex flex-col relative bg-white md:bg-transparent">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 mb-6 md:mb-8 shrink-0 px-4 md:px-0 mt-4 md:mt-0">
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <div className="px-6 py-3 bg-neutral-900 rounded-2xl text-white flex items-center gap-4 shadow-xl shrink-0"><ShieldCheck className="h-5 w-5 text-emerald-400" /><div><p className="text-[8px] font-black uppercase tracking-widest opacity-40 leading-none">Net Kharchu</p><p className="text-xl font-black tracking-tight text-white">₹{totalFarmExpenses.toLocaleString()}</p></div></div>
        </div>
      </div>
